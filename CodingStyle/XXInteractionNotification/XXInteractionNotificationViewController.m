//
//  XXInteractionNotificationViewController.m
...
#import "XXInteractionNotificationViewController.h"
...
@interface XXInteractionNotificationViewController () <UITableViewDataSource, UITableViewDelegate>
...
@implementation XXInteractionNotificationViewController
...
- (void)networkAction:(NSInteger)pageNo refresh:(BOOL)refresh {
    ...
    [self networkActionWithpageNo:@(pageNo) pageSize:@(kXXInteractionNotificationPageSize) succeededHandler:^(XXInteractionNotification *notification) {
        ...
        if (0 == notification.msgList.count) {
            // show no content
            return;
        }
        ...
        for (int i = 0; i < notification.msgList.count; ++i) {
            ...
            XXInteractionNotificationLayout *layout = [[XXInteractionNotificationLayout alloc] initWithInteractionNotificationItem:item type:item.type.integerValue entryType:XXInteractionNotificationEntryTypeMain];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (layout.interactionNotificationType == XXInteractionNotificationTypePraise) {
        XXPraiseDetailViewController *praiseDetailVC = [XXPraiseDetailViewController new];
        [self.navigationController pushViewController:praiseDetailVC animated:YES];
    }
    
    if (layout.interactionNotificationType == XXInteractionNotificationTypeArticalCommentReply) {
